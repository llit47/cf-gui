...
def create_app(*, config_path: str | None = None, password_hash: str | None = None,
    ...
    def index():
        document = store.load()
        return render_template("index.html", entries=store.entries(document),
                               tunnel=document.get("tunnel", "(brak w config.yml)"))

    @app.get("/ingress/new")
    @login_required
    def new_entry():
        return render_template("entry_form.html", title="Dodaj wpis", action=url_for("add_entry"),
                               hostname="", service="", is_new=True)
    ...
    def add_entry():
        ...
        try:
            store.add(document, hostname, request.form.get("service", ""))
            backup = store.save(document)
        except ConfigError as exc:
            flash(str(exc), "error")
            return render_template("entry_form.html", title="Dodaj wpis", action=url_for("add_entry"),
                                   hostname=hostname, service=request.form.get("service", ""), is_new=True), 400
        flash(f"Wpis zapisany. Backup: {backup or 'nie było poprzedniego pliku'}.", "success")
        if request.form.get("create_dns"):
            _route_dns(document, hostname)
        return redirect(url_for("index"))
    ...
    def edit_entry(index: int):
        document = store.load()
        try:
            item = store._entry(document, index)
        except ConfigError:
            abort(404)
    ...
    def update_entry(index: int):
        ...
        try:
            store.edit(document, index, hostname, service)
            backup = store.save(document)
        except ConfigError as exc:
            flash(str(exc), "error")
            return render_template("entry_form.html", title="Edytuj wpis", action=url_for("update_entry", index=index),
                                   hostname=hostname, service=service, is_new=False), 400
        flash(f"Wpis zapisany. Backup: {backup}.", "success")
        return redirect(url_for("index"))
    ...
    def delete_entry(index: int):
        document = store.load()
        try:
            store.delete(document, index)
            store.save(document)
        except ConfigError as exc:
            flash(str(exc), "error")
            return redirect(url_for("index"))
        flash("Wpis usunięty; backup konfiguracji utworzony.", "success")
        return redirect(url_for("index"))
    ...
    def route_entry_dns(index: int):
        document = store.load()
        try:
            hostname = store._entry(document, index)["hostname"]
        except ConfigError:
            abort(404)
        _route_dns(document, hostname)
        return redirect(url_for("index"))
