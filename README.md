...
3. Dodaj, edytuj lub usuń wpis. Formularz wymaga domenowej nazwy `hostname` (dopuszcza `*.example.com`) i poprawnego `service`. Adres originu bez schematu, np. `origin:8096` lub `192.0.2.79:8080`, dostaje prefiks `http://`; adresy originu ze ścieżką, query lub fragmentem są odrzucane. Jawne schematy, Unix socket (`unix:/...`, `unix+tls:/...`) oraz usługi specjalne (`bastion`, `hello_world`, `http_status:100–999`) pozostają bez zmian. Błędne dane są odrzucane przed przygotowaniem candidate. Aplikacja waliduje zmianę także przez `cloudflared`, tworzy backup, aktywuje config i automatycznie restartuje cloudflared. Panel pokazuje jeden z trzech wyników: sukces, nieudana aktywacja z udanym rollbackiem albo nieudana aktywacja z nieudanym rollbackiem. Po sukcesie używa krótkiego komunikatu; przy obu rodzajach błędu pełną diagnostykę pokazuje bezpośrednio na stronie, bez zapisywania jej w sesji cookie.
